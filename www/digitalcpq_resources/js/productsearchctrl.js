var searchCtrl =  function ($resource, $scope, $rootScope, $location, $timeout, SFDCData) {
	...
	/* new search */
	$scope.facetorder = ['khdev__Type__c', 'khdev__Make__c', 'khdev__Available_Tariffs__c', 'khdev__Operating_system__c', 'khdev__Colour__c'];
	$scope.search = function (stxt) {
		...
		// run search
    	SFDCData.query("khdev__Product__c", ["Id", "Name", "khdev__Type__c", "khdev__Make__c", "khdev__Available_Tariffs__c", "khdev__Operating_system__c", "khdev__Colour__c", "khdev__ThumbImageB64__c"],  filters)
    	  .then(function (data) {
    		...
    		for (var ridx in data) {
    			...
    			if (rec.khdev__Available_Tariffs__c) {
    				rec.khdev__Available_Tariffs__c = rec.khdev__Available_Tariffs__c.split(',');
    			}
